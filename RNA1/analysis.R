...
tmp = prcomp(norm$hsa, center=T, scale.=T)
pca$hsa = merge(meta, tmp$x[,c("PC1", "PC2")], by=0)
...
# Human only PCA
p1=ggplot(pca$hsa, aes(x=PC1, y=PC2, color=Treatment))+
  geom_point()+
  ggtitle("Human")
p2=ggplot(pca$hsa, aes(x=PC1, y=PC2, color=CellLine))+
  geom_point()+
  ggtitle("Human")
ps = align_patches(p1, p2)
pdf(paste0(paths$out, "/pca_hsa.pdf"), width=0.5*w_in, height=0.25*h_in)
ps
dev.off()
...
# The decision on whether to use the intersection or union of expressed genes was 
# based on the number of DE genes and whether the baseline GSEA showed any enrichemnt
...
dge = estimateDisp(dge, design)
fit = glmFit(dge, design)
# Get Aco vs Hsa results
conts = makeContrasts("SpeciesACO - SpeciesHSA", levels=design)
res_aco_vs_hsa = as.data.frame(glmLRT(fit, contrast=conts))
res_aco_vs_hsa$PAdj = p.adjust(res_aco_vs_hsa$PValue, method="BH")
...
# Combine T1 results
des_baseline$T1 = res_aco_vs_hsa
colnames(des_baseline$T1) = paste0(colnames(des_baseline$T1), "_aco_vs_hsa")
tmp = merge(res_aco_vs_mus, res_aco_vs_musw, by=0, suffixes=c("_aco_vs_mus", "_aco_vs_musw"))
des_baseline$T1 = merge(des_baseline$T1, tmp, by.x=0, by.y="Row.names") %>%
  dplyr::select(-starts_with("LR")) %>%
  column_to_rownames("Row.names")
colSums(des_baseline$T1[c("PAdj_aco_vs_hsa", "PAdj_aco_vs_mus", "PAdj_aco_vs_musw")] < 0.05)
...
rm(res_aco_vs_hsa, res_aco_vs_mus, res_aco_vs_musw)
...
des_baseline$T1 = des_baseline$T1 %>%
  ...
  mutate(sig_aco_vs_hsa = interaction(sig_aco_vs_hsa, up_aco_vs_hsa)) %>%
  ...
  mutate(sig_aco_vs_hsa = fct_recode(sig_aco_vs_hsa, "Up" = "TRUE.TRUE", "Down" = "TRUE.FALSE", "NotSig" = "FALSE.FALSE"))
...
p4 = my_volcano(des_treatment$hsa_24h, col="sig_col10", v1="logFC_10vsC", v2="PValue_10vsC", "Human")

p5 = my_volcano(des_treatment$aco_24h, col="sig_col20", v1="logFC_20vsC", v2="PValue_20vsC")
p6 = my_volcano(des_treatment$mus_24h, col="sig_col20", v1="logFC_20vsC", v2="PValue_20vsC")
p7 = my_volcano(des_treatment$musw_24h, col="sig_col20", v1="logFC_20vsC", v2="PValue_20vsC")
p8 = my_volcano(des_treatment$hsa_24h, col="sig_col20", v1="logFC_20vsC", v2="PValue_20vsC")
...
p3 = my_volcano(des_baseline$T1, col="sig_aco_vs_hsa", v1="logFC_aco_vs_hsa", v2="PValue_aco_vs_hsa", "Acomys vs Human")
...
res = des_treatment$musw_24h %>%
  ...
  mutate(Species = "musw") %>%
  rbind(res, .)
res = des_treatment$hsa_24h %>%
  dplyr::select(MouseSymbol, logFC_10vsC:PAdj_20vsC) %>%
...
des_combined$res24h_10vsC = des_baseline$T1[genes_intersection, ] %>%
  dplyr::select(logFC_aco_vs_hsa:PAdj_aco_vs_musw) %>%
  cbind(des_combined$res24h_10vsC[genes_intersection, ], .)
des_combined$res24h_20vsC = des_baseline$T1[genes_intersection, ] %>%
  dplyr::select(logFC_aco_vs_hsa:PAdj_aco_vs_musw) %>%
  cbind(des_combined$res24h_20vsC[genes_intersection, ], .)
...
des_combined$res12d_10vsC = des_baseline$T2[genes_intersection, ] %>%
  dplyr::select(logFC_aco_vs_mus:PAdj_aco_vs_musw) %>%
  cbind(des_combined$res12d_10vsC[genes_intersection, ], .)
des_combined$res12d_20vsC = des_baseline$T2[genes_intersection, ] %>%
  dplyr::select(logFC_aco_vs_mus:PAdj_aco_vs_musw) %>%
  cbind(des_combined$res12d_20vsC[genes_intersection, ], .)
...
# 
# # --- Human ---
# gse_treatment$hsa24h_10gy = my_gse(des_treatment[["hsa_24h"]], "logFC_10vsC", orgdb = org.Hs.eg.db)
# gse_treatment$hsa24h_20gy = my_gse(des_treatment[["hsa_24h"]], "logFC_20vsC", orgdb = org.Hs.eg.db)
# 
# # --- Mouse ---
...
ps[[7]]=gse_treatment$hsa24h_10gy %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign)+
  theme(axis.text.y = element_text(size = 10))+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 100))+
  ggtitle("Human 10gy vs C")
ps[[8]]=gse_treatment$hsa24h_20gy %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign)+
  theme(axis.text.y = element_text(size = 10))+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 100))+
  ggtitle("Human 20gy vs C")
ps = align_patches(ps)
...
data.frame(cor(gse_combined$T1[, 3:10], use = "pairwise.complete.obs")) %>%
...
gse_combined$T1$padj_hsa = pmin(gse_combined$T1$padj_hsa24h_10gy, gse_combined$T1$padj_hsa24h_20gy, na.rm = T)
...
gse_combined$T1$padj_min = pmin(gse_combined$T1$padj_hsa, gse_combined$T1$padj_mus, 
                           gse_combined$T1$padj_musw, gse_combined$T1$padj_aco,
                           na.rm=T)
...
for (p in 1:ceiling(nrow(gse_any_sig$T1)/n_per_page)) {
  ...
  page_ps = list()
  tmp = gse_any_sig$T1[page_first:min(page_first+n_per_page-1, nrow(gse_any_sig$T1)), 1:18] %>%
...
# 
# # --- Aco vs Hsa
# gse_baseline$aco_vs_hsa_24h = my_gse(des_baseline$T1, "logFC_aco_vs_hsa", org.Mm.eg.db)
# 
# # --- Aco vs Mus
...
facet_titles = c(
  ...
  suppressed = "Higher in Human"
)
p1=gse_baseline$aco_vs_hsa_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 100))+
  ggtitle("Acomys vs Human Baseline")

facet_titles = c(
  activated = "Higher in Acomys",
  suppressed = "Higher in Mouse"
)
p2=gse_baseline$aco_vs_mus_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
...
p3=gse_baseline$aco_vs_musw_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
...
# 
# # --- Aco vs Hsa ---
# gse_diff_response$aco_vs_hsa10_24h = des_combined$res24h_10vsC %>%
#   mutate(logFCdiff = logFC_10vsC_aco - logFC_10vsC_hsa) %>%
#   my_gse(., "logFCdiff", org.Mm.eg.db)
# gse_diff_response$aco_vs_hsa20_24h = des_combined$res24h_20vsC %>%
#   mutate(logFCdiff = logFC_20vsC_aco - logFC_20vsC_hsa) %>%
#   my_gse(., "logFCdiff", org.Mm.eg.db)
# 
# # --- Aco vs Mus ---
...
p1=gse_diff_response$aco_vs_hsa10_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 100))+
  ggtitle("Acomys vs Human 10gy")
p2=gse_diff_response$aco_vs_hsa20_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 100))+
  ggtitle("Acomys vs Human 20gy")

facet_titles = c(
  activated = "Higher in Acomys",
  suppressed = "Higher in Mouse"
)
p3=gse_diff_response$aco_vs_mus10_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
...
p4=gse_diff_response$aco_vs_mus20_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
...
p5=gse_diff_response$aco_vs_musw10_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
...
p6=gse_diff_response$aco_vs_musw20_24h %>%
  dplyr::filter(p.adjust < 0.05) %>%
  dotplot(., split=".sign", showCategory = 20)+
  facet_grid(.~.sign, labeller=as_labeller(facet_titles))+
  theme(axis.text.y = element_text(size = 10))+
...
ps = align_patches(p1, p2, p3, p4, p5, p6)
...
gse_diff_sig = list()
gse_diff_sig$T1 = gse_any_sig$T1[, 1:18] %>% 
  pivot_longer(
    names_to = c("Variable", "Species", "Timepoint", "Treatment"),
    names_pattern = "(.*)_(hsa|mus|musw|aco)(24h)_(10gy|20gy)",
    cols = -c(ID, Description)) %>%
...
meta %>%
  ...
  mutate(Species = fct_recode(Species, "A. cah" = "ACO", "M. mus" = "MUS", "M. mus W" = "MUSW")) %>%
  mutate(Treatment = factor(Treatment, levels=c("Con", "10Gy", "20Gy")))%>%
  ...
  theme(axis.title.x = element_blank())
ggsave(paste0(paths$out, "/detail_telo/tert.pdf"), width = 1*w, height=0.3*h, units="mm")

setdiff(rownames(counts$union), rownames(a))
meta[rownames(counts$union), ] %>%
  mutate(Tert = norm$union[rownames(counts$union), "Tert"]) %>%
  ggplot(., aes(Species, Tert))+
  geom_boxplot()
ggsave(paste0(paths$out, "/detail_telo/tert_with_hsa.png"))
...
plt = wrap_plots(ps, guides = "collect", ncol=4)&
  theme(axis.title=element_text(size=8),
        ...
        legend.position = "bottom",
        legend.box = "vertical")
ggsave(plot=plt, paste0(paths$out, "/detail_telo/go_terms_zcores_complete.pdf"), width=1.2*w, height=2.2*h, units="mm")
...
plt = wrap_plots(ps, guides = "collect", ncol=4)&
  theme(axis.title=element_text(size=8),
        ...
        legend.position = "bottom",
        legend.box = "vertical")
ggsave(plot=plt, paste0(paths$out, "/detail_telo/go_terms_zcores_nohsa24.pdf"), width=1*w, height=2*h, units="mm")

all.equal(rownames(telo_zscores), rownames(this_meta))
meanZs = data.frame()
for (goid in unique(go_members$GO)) {
  godesc = go_sets[go_sets$go_id == goid, "Term"]
  genes_oi = go_members %>%
    ...
    dplyr::filter(SYMBOL %in% colnames(telo_zscores)) %>%
    pull(SYMBOL)
  meanZs = rbind(meanZs, this_meta %>%
    dplyr::select(Species) %>%
    mutate(zscore = rowMeans(telo_zscores[, genes_oi]))%>%
    group_by(Species) %>%
    summarize_all(mean) %>%
    mutate(Description = godesc, .before=zscore))
}

order = meanZs %>%
  dplyr::filter(Species == "ACO") %>%
  arrange(zscore)%>%
  pull(Description)
meanZs %>%
  mutate(Description = factor(Description, levels=order)) %>%
  ggplot(., aes(Species, Description, fill=zscore))+
  geom_tile()+
  scale_fill_gradient2(low="blue", high="red")
...
gse_combined$T1 %>%
  ...
  drop_na()%>%
  dplyr::select(-c(padj_hsa:padj_min)) %>%
  mutate(Description = fct_reorder(Description, NES_aco24h_10gy))%>%
  pivot_longer(cols=-c(ID, Description)) %>%
  mutate(Species = str_extract(name, "_(mus|aco|musw|hsa)24h_.*", group=1)) %>%
  mutate(Dose = str_extract(name, "_(10gy|20gy)", group=1)) %>%
  ...
  theme(axis.title = element_blank())
ggsave(paste0(paths$out, "/detail_telo/go_terms_gsea24h.pdf"), width=1.2*w, height=0.8*h, units="mm")

gse_combined$T2 %>%
  dplyr::filter(ID %in% go_sets$go_id) %>%
  drop_na()%>%
  dplyr::select(-c(padj_mus:padj_min)) %>%
  mutate(Description = fct_reorder(Description, NES_aco12d_10gy))%>%
  pivot_longer(cols=-c(ID, Description)) %>%
  mutate(Species = str_extract(name, "_(mus|aco|musw)12d_.*", group=1)) %>%
  mutate(Dose = str_extract(name, "_(10gy|20gy)", group=1)) %>%
  mutate(name = str_remove(name, "_.*")) %>%
  pivot_wider(names_from = "name") %>%
  mutate(sig = stars.pval(padj))%>%
  ...
  scale_y_discrete(expand=c(0,0))+
  theme(axis.title = element_blank())
ggsave(paste0(paths$out, "/detail_telo/go_terms_gsea12d.pdf"), width=1.2*w, height=0.8*h, units="mm")
...
setdiff(aco_nes24h$ID, aco_nes24h_nohsa$ID)

aco_nes24h_nohsa %>%
  # mutate(Description = ifelse(nchar(Description) > 50, paste0(substr(Description, 1, 50), "..."), Description))%>%
  mutate(Description = fct_reorder(Description, NES_aco24h_10gy)) %>%
  pivot_longer(cols=-c(ID, Description), names_pattern = "(.*)_(.*)24h_(.*)", names_to = c("Var", "Species", "Dose")) %>%
  dplyr::filter(Species != "hsa") %>%
  pivot_wider(names_from = Var, values_from = value) %>%
  mutate(Species = factor(Species, levels=c("aco", "musw", "mus"))) %>%
  mutate(Species = fct_recode(Species, "A. car" = "aco", "M. mus W" = "musw", "M. mus" = "mus")) %>%
  mutate(Dose = factor(Dose, levels=c("10gy", "20gy"))) %>%
  arrange(Species, Dose) %>%
  # mutate(Condition = paste(Species, Dose, "vsC", sep="_")) %>%
  # mutate(Condition = factor(Condition, levels = unique(Condition))) %>%
  ...
  theme(axis.text.x = element_text(angle=0, hjust=0.5),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_gsea_heatmap24h_nohsa.pdf"), width = 1.3*w, height = 0.5*h, units="mm")
...
aco_nes12d_fewna %>%
  ...
  theme(axis.text.x = element_text(angle=0, hjust=0.5),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_gsea_heatmap12d.pdf"), width = 1.3*w, height = 0.3*h, units="mm")

# Checking if anything related to these topics has extreme NES differences
th_pval_nes = 0.05 # 0.05
th_pval_nes_high = 0.1
aco_nes24h_relaxed = gse_combined$T1 %>%
  dplyr::filter(padj_aco24h_10gy < th_pval_nes & padj_aco24h_20gy < th_pval_nes) %>%
  dplyr::filter((is.na(NES_mus24h_10gy) | (padj_mus24h_10gy > th_pval_nes_high) | NES_mus24h_10gy * NES_aco24h_10gy < 0) &
                  (is.na(NES_mus24h_20gy) | (padj_mus24h_20gy > th_pval_nes_high) | NES_mus24h_20gy * NES_aco24h_20gy < 0)) %>%
  dplyr::filter((is.na(NES_musw24h_10gy) | (padj_musw24h_10gy > th_pval_nes_high) | NES_musw24h_10gy * NES_aco24h_10gy < 0) &
                  (is.na(NES_musw24h_20gy) | (padj_musw24h_20gy > th_pval_nes_high)| NES_musw24h_20gy * NES_aco24h_20gy < 0)) %>%
  dplyr::select(ID:padj_aco24h_20gy)
aco_nes24h_relaxed = aco_nes24h_relaxed[rowSums(is.na(aco_nes24h_relaxed))==0, ]
aco_nes12d_relaxed = gse_combined$T2 %>%
  dplyr::filter(padj_aco12d_10gy < th_pval_nes & padj_aco12d_20gy < th_pval_nes) %>%
  dplyr::filter((is.na(NES_mus12d_10gy) | (padj_mus12d_10gy > th_pval_nes_high) | NES_mus12d_10gy * NES_aco12d_10gy < 0) &
                  (is.na(NES_mus12d_20gy) | (padj_mus12d_20gy > th_pval_nes_high) | NES_mus12d_20gy * NES_aco12d_20gy < 0)) %>%
  dplyr::filter((is.na(NES_musw12d_10gy) | (padj_musw12d_10gy > th_pval_nes_high) | NES_musw12d_10gy * NES_aco12d_10gy < 0) &
                  (is.na(NES_musw12d_20gy) | (padj_musw12d_20gy > th_pval_nes_high)| NES_musw12d_20gy * NES_aco12d_20gy < 0)) %>%
  dplyr::select(ID:padj_aco12d_20gy)
aco_nes12d_relaxed = aco_nes12d_relaxed[rowSums(is.na(aco_nes12d_relaxed))==0, ]
...
aco_nes24h_relaxed %>%
  ...
  theme(axis.text.x = element_text(angle=0, hjust=0.5),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_gsea_heatmap24h_relaxed.pdf"), width = 1.3*w, height = 5*h, units="mm")
...
aco_nes12d_relaxed %>%
  ...
  theme(axis.text.x = element_text(angle=0, hjust=0.5),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_gsea_heatmap12d_relaxed.pdf"), width = 1.3*w, height = 2*h, units="mm")
...
setdiff(selected_terms$Description, aco_nes24h_relaxed$Description)
intersect(selected_terms$Description, aco_nes24h$Description)
length(intersect(selected_terms$Description, aco_nes24h$Description))
length(aco_nes24h$Description)

aco_nes24h_relaxed %>%
  merge(., selected_terms, by="Description")%>%
  mutate(Description = ifelse(nchar(Description) > 80, paste0(substr(Description, 1, 80), "..."), Description))%>%
  mutate(Description = fct_reorder(Description, NES_aco24h_10gy)) %>%
  pivot_longer(cols=-c(ID, Description, Topic), names_pattern = "(.*)_(.*)24h_(.*)", names_to = c("Var", "Species", "Dose")) %>%
  dplyr::filter(Species != "hsa") %>%
  pivot_wider(names_from = Var, values_from = value) %>%
  mutate(Species = factor(Species, levels=c("aco", "musw", "mus"))) %>%
  mutate(Species = fct_recode(Species, "A. cah" = "aco", "M. mus W" = "musw", "M. mus" = "mus")) %>%
  mutate(Dose = factor(Dose, levels=c("10gy", "20gy"))) %>%
  arrange(Species, Dose) %>%
  # mutate(Condition = paste(Species, Dose, "vsC", sep="_")) %>%
  # mutate(Condition = factor(Condition, levels = unique(Condition))) %>%
  ...
  geom_tile()+
  geom_text(color="white")+
  facet_grid(cols=vars(Dose), rows=vars(Topic), scale="free", space="free")+
  scale_fill_gradient2(low="blue", high="red")+
  scale_x_discrete(expand = c(0,0))+
  scale_y_discrete(expand = c(0,0))+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/fig2d_v2.pdf"), width = 1*w, height=2*h, units="mm")

##### Aco specific genes 24h #####
...
# original aco_de24h_fewna contained 17 genes
...
#   coord_cartesian(xlim=c(-4,4), ylim=c(-4,4))
# wrap_plots(ps, ncol=2)+plot_layout(guides="collect")
# ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_filtering24h.pdf"), width = w, height = 0.75*h, units="mm")

aco_de24h = aco_de24h %>%
  dplyr::filter(Passes) %>%
  dplyr::select(-contains("HSA"))
aco_de24h_fewna = aco_de24h[rowSums(is.na(aco_de24h))<=4, ] # Can be NA in one species (2x logFC, 2x pvals)
...
aco_de24h_fewna %>%
  ...
  pivot_longer(cols=-c(GeneSymbol, Timepoint), names_pattern = "(.*)_(.*)_(.*)", names_to = c("Var", "Comparison", "Species")) %>%
  pivot_wider(names_from = Var, values_from = value) %>%
  mutate(Species = factor(tolower(Species), levels=c("aco", "musw", "mus"))) %>%
  mutate(Species = fct_recode(Species, "A. cah" = "aco", "M. mus W" = "musw", "M. mus" = "mus")) %>%
  arrange(Species, Comparison) %>%
  mutate(Condition = paste(Species, Comparison, sep="_")) %>%
  mutate(Condition = factor(Condition, levels = unique(Condition))) %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap24h_nohsa_full.pdf"), width = 0.75*w, height = 0.5*h, units="mm")
aco_de24h_fewna %>%
  drop_na() %>%
  ungroup() %>%
  dplyr::select(-Passes) %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap24h_nohsa_full_nona.pdf"), width = 0.75*w, height = 0.5*h, units="mm")

aco_de24h_fewna %>%
  ungroup() %>%
  dplyr::select(-Passes) %>%
  mutate(signLogFC = sign(logFC_10vsC_ACO)) %>%
  group_by(signLogFC) %>%
  top_n(abs(logFC_10vsC_ACO), n=5)%>%
  ungroup()%>%
  dplyr::select(-signLogFC)%>%
  mutate(GeneSymbol = fct_reorder(GeneSymbol, logFC_10vsC_ACO)) %>%
  pivot_longer(cols=-c(GeneSymbol, Timepoint), names_pattern = "(.*)_(.*)_(.*)", names_to = c("Var", "Comparison", "Species")) %>%
  pivot_wider(names_from = Var, values_from = value) %>%
  mutate(Species = factor(tolower(Species), levels=c("aco", "musw", "mus"))) %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap24h_nohsa_top5.pdf"), width = 0.75*w, height = 0.3*h, units="mm")
aco_de24h_fewna %>%
  drop_na()%>%
  ungroup() %>%
  dplyr::select(-Passes) %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap24h_nohsa_top5_nona.pdf"), width = 0.75*w, height = 0.3*h, units="mm")
...
aco_de24h = comp24h_wide %>%
  dplyr::filter(abs(logFC_10vsC_ACO) > th_high & PAdj_10vsC_ACO < th_pval_low) %>%
  dplyr::filter(abs(logFC_20vsC_ACO) > th_high & PAdj_20vsC_ACO < th_pval_low) %>%
  dplyr::filter(is.na(logFC_10vsC_MUS) | (abs(logFC_10vsC_MUS) < th_low & PAdj_10vsC_MUS > th_pval_high)| logFC_10vsC_MUS * logFC_10vsC_ACO < 0) %>%
  dplyr::filter(is.na(logFC_20vsC_MUS) | (abs(logFC_20vsC_MUS) < th_low & PAdj_20vsC_MUS > th_pval_high)| logFC_20vsC_MUS * logFC_20vsC_ACO < 0) %>%
  dplyr::filter(is.na(logFC_10vsC_MUSW) | (abs(logFC_10vsC_MUSW) < th_low & PAdj_10vsC_MUSW > th_pval_high)| logFC_10vsC_MUSW * logFC_10vsC_ACO < 0) %>%
  dplyr::filter(is.na(logFC_20vsC_MUSW) | (abs(logFC_20vsC_MUSW) < th_low & PAdj_20vsC_MUSW > th_pval_high)| logFC_20vsC_MUSW * logFC_20vsC_ACO < 0) %>%
  dplyr::select(-contains("HSA"))
aco_de24h_fewna = aco_de24h[rowSums(is.na(aco_de24h))<=4, ] # Can be NA in one species (2x logFC, 2x pvals)
# Original aco_de24h_fewna contained 17 genes
# Same as above: 27 genes

##### Aco specific genes 12d #####

# ps = list()
# Original aco_de12d: 387
# Original aco_de12d_fewna: 155
aco_de12d = comp12d_wide %>%
  mutate(Passes = abs(logFC_10vsC_ACO) > th_high & PAdj_10vsC_ACO < th_pval)
# ps[[1]] = ggplot(aco_de12d, aes(logFC_10vsC_ACO, logFC_10vsC_MUS, color=Passes))+
#   geom_point(size=0.5)+
...
#   coord_cartesian(xlim=c(-4,4), ylim=c(-4,4))
# wrap_plots(ps, ncol=2)+plot_layout(guides="collect")
# ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_filtering12d.png"), width = w, height = 0.8*h, units="mm")
...
aco_de12d_fewna %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap12d_full.pdf"), width = 0.75*w, height = 2*h, units="mm")
aco_de12d_fewna %>%
  drop_na() %>%
  ungroup() %>%
  dplyr::select(-Passes) %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap12d_full_nona.pdf"), width = 0.75*w, height = 1.7*h, units="mm")
...
aco_de12d_fewna %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap12d_top5.pdf"), width = 0.75*w, height = 0.3*h, units="mm")
aco_de12d_fewna %>%
  drop_na()%>%
  ungroup() %>%
  dplyr::select(-Passes) %>%
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/aco_specific_genes_heatmap12d_top5_nona.pdf"), width = 0.75*w, height = 0.3*h, units="mm")
...
aco_de12d2 = comp12d_wide %>%
  ...
  dplyr::filter(is.na(logFC_10vsC_MUSW) | (abs(logFC_10vsC_MUSW) < th_low & PAdj_10vsC_MUSW > th_pval_high)| logFC_10vsC_MUSW * logFC_10vsC_ACO < 0) %>%
  dplyr::filter(is.na(logFC_20vsC_MUSW) | (abs(logFC_20vsC_MUSW) < th_low & PAdj_20vsC_MUSW > th_pval_high)| logFC_20vsC_MUSW * logFC_20vsC_ACO < 0)
aco_de12d_fewna2 = aco_de12d2[rowSums(is.na(aco_de12d2))<=4, ] # Can be NA in one species (2x logFC, 2x pvals)
# Original aco_de12d_fewna contained 155 genes
# Now 147, very similar
...
tmp1 = res_custom_gsets$sen_mayo$gsea %>%
  ...
  mutate(Species = factor(Species, levels=c("aco", "musw", "mus", "hsa"))) %>%
  mutate(Species = fct_recode(Species, "A. car" = "aco", "M. mus W" = "musw", "M. mus" = "mus")) %>%
  mutate(Timepoint = str_remove(Context, ".+_")) %>%
  mutate(Dose = fct_recode(Comparison, "10gy" = "10vsC", "20gy" = "20vsC"))
# --- Fathima custom ---
...
tmp %>%
  ...
  geom_tile()+
  geom_text(color="white")+
  facet_grid(cols=vars(Dose), rows=vars(Source), scales = "free", space="free")+
  scale_fill_gradient2(low="blue", high="red")+
  scale_x_discrete(expand = c(0,0))+
  scale_y_discrete(expand = c(0,0))+
  theme(axis.text.x = element_text(angle=0, hjust=0.5),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/sen_sigs24h.pdf"), width = 1.1*w, height = 0.5*h, units="mm")
tmp %>%
  # dplyr::filter(Species != "hsa") %>%
  dplyr::filter(Timepoint == "12d") %>%
  # dplyr::filter(Description != "Protein modifying enzyme") %>%
  ...
  theme(axis.text.x = element_text(angle=0, hjust=0.5),
        axis.title = element_blank())
ggsave(paste0(paths$out, "/paper_figs/sen_sigs12d.pdf"), width = 1.1*w, height = 0.5*h, units="mm")
...
meta %>%
  ...
  theme(axis.title.x = element_blank())
ggsave(paste0(paths$out, "/paper_figs/p21.pdf"), width = 0.8*w, height=0.3*h, units="mm")
...
tmp = rbind(
  comp24h %>% 
    pivot_longer(cols=contains("vsC"), names_to = c(".value", "comparison"), names_sep="_") %>%
    mutate(Timepoint = "24h"),
  comp12d %>% 
    pivot_longer(cols=contains("vsC"), names_to = c(".value", "comparison"), names_sep="_") %>%
    mutate(Timepoint = "12d"))
p2 = tmp %>%
  dplyr::filter(GeneSymbol == "Cdkn1a") %>%
  dplyr::filter(Species != "HSA") %>%
  mutate(Species = fct_recode(Species, "A. cah" = "ACO", "M. mus" = "MUS", "M. mus W" = "MUSW")) %>%
  mutate(Timepoint = factor(Timepoint, levels=c("24h", "12d")))%>%
  mutate(sig = stars.pval(PAdj)) %>%
  ggplot(., aes(Species, logFC, fill=comparison, label= sig))+
...
p1+p2 &
  theme(legend.position = "bottom", 
        axis.text.x = element_text(angle=45, hjust=1))
ggsave(paste0(paths$out, "/paper_figs/p21_v2.pdf"), width = 1*w, height=0.4*h, units="mm")

# Matches visuals
des_baseline$T1["Cdkn1a", ]
des_baseline$T2["Cdkn1a", ]
...
#   theme(axis.title.x = element_blank())
# ggsave(paste0(paths$out, "/paper_figs/p16.pdf"), width = 0.8*w, height=0.3*h, units="mm")
...
# Custom gene sets
write.table(res_custom_gsets$sen_mayo$gsea, paste0(paths$tables, "/gsea_treatment_sen_mayo.tsv"), 
            row.names = F, sep="\t", quote=F)
write.table(res_custom_gsets$fathima_custom$gsea, paste0(paths$tables, "/gsea_treatment_sen_custom.tsv"), 
            row.names = F, sep="\t", quote=F)
...
write.csv(counts$musw, paste0(paths$tables, "/counts_musw.csv"))
